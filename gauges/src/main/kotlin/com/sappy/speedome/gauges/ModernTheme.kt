...
object ModernTheme : GaugeTheme {
    ...
    override fun DrawScope.drawStatic(frame: GaugeFrame, assets: GaugeAssets) {
        ...
        for (set in frame.scaleSets()) {
            ...
            while (v <= set.max + 1e-3f && v <= frame.rangeKmh * 1.001f) {
                ...
                drawCircle(Color.White.copy(alpha = .35f * set.alpha), r * .008f + 1.dp.toPx() / 2, polar(c, r * 1.12f, a))
                val p = polar(c, r * .8f, a)
                text(fmt(v.toDouble(), 0), p.x, p.y, labels, r * .075f, Color.White.copy(alpha = .42f * set.alpha))
                v += step.label
            }
        }
    ...
    override fun DrawScope.drawDynamic(frame: GaugeFrame, assets: GaugeAssets) {
        ...
        if (frame.needleKmh > .05f) {
            ...
            val brush = assets.memo("modern-arc-${accent.value}", size) {
                Brush.sweepGradient(0f to accent.copy(alpha = .25f), .75f to accent, 1f to accent, center = c)
            }
            rotate(DIAL_START_DEG, c) {
                // soft glow: wider, fainter passes under the arc
                drawArc(accent.copy(alpha = .08f), 0f, sweep, false, box, boxSize, style = Stroke(lw * 2.4f, cap = StrokeCap.Round))
                drawArc(accent.copy(alpha = .12f), 0f, sweep, false, box, boxSize, style = Stroke(lw * 1.7f, cap = StrokeCap.Round))
