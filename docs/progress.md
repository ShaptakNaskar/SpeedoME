...
3. Use `tools/emu.sh` for emulator checks: `start`, `install`, `setup-dev` (grants permissions, turns on developer options and the simulator), `tap "Text"`, `texts`, `shot NAME` (to /tmp/speedome-shots), `feed KMH SECONDS`, `profile KMH:S …` (one continuous drive through several speeds), `crashes`. uiautomator escapes `&` as `&amp;` in texts. Vibrations (the speed limit) show in `adb shell dumpsys vibrator_manager`. The emulator runs at font scale 1.3, where the Speed controls drop their labels; `settings put system font_scale 1.0` shows the labelled row.
...
|---|---|---|
| 0.9.1 | 2026-09-24 | Step counting, map pinch, sweep and G-force fixes; Drive/Walk and Target buttons (commit e6299fa). |
| 0.10.0 | 2026-09-26 | Owner's requests: (1) trip detail shows the route on the dark MapLibre map (start/finish dots, fit-route button, speed legend; the canvas route stays as the offline fallback); (2) the live meter runs only while the app is on screen: leaving it without recording sends `Command.Standby` (a clean live meter, GPS off, engine ticks and screen polling stopped), and the foreground service runs only while a trip records (D27); (3) speed limit: scales fixed at 125 % of it (rounded up to 5), red zone on every theme, red fade over the last 10 %, one buzz at the limit then 1/2/3/4 Hz pulses (D28); (4) the Retro odometer rolls continuously between fixes; (5) Synthwave removed. Verified on the emulator: limit dialog, red zones, the vibration log (`dumpsys vibrator_manager`) for every band, controls row at 1.0× and 1.3× font, trip map, and nothing running after leaving the app. |
...
- **Live meter lifecycle (0.10):** `AppContainer.trackingActive` = activity started || recording service in the foreground. Sensors, the engine's 1 Hz tick, the simulator and the speed-limit buzz follow it. When it turns false the engine gets `Command.Standby`, which a trip ignores and a live meter answers by dropping everything except the learned stride. The recording service starts when the app is on screen and a trip is active (the coordinator in `AppContainer`), and stops itself once no trip is active, after `SessionRecorder.ready` so a sticky restart can restore the trip first. Live meters are no longer stored in Room; rows left by older versions are deleted on launch. Screen polling (`rememberTrackView`, battery, Nerd ticks) runs only while the activity is started.
- **Speed limit (0.10):** the limit is stored in m/s (0 = off). `GaugeDriver` rounds it to 0.1 in display units and fixes the range at `SpeedLimit.scaleMax` (125 %, up to a multiple of 5), which overrides auto-range and Night Focus's dial; the engine's own auto-range keeps running underneath. Scales between rungs tick every 5 (`niceStep`), and bar themes label them with the dial step (`barLabels`). The colour fade follows the needle (`GaugeFrame.limitWarn`); the red zone is in the static layer (`StaticKey.limitKmh`). The buzz follows the engine's filtered speed through `LimitAlarm` (pure, in `:engine`) and one repeating waveform per change, with alarm usage so it works with the ringer silent. Vibrations from our uid show in `adb shell dumpsys vibrator_manager`, which is how the emulator runs were checked.
- **Maps inside scrolling pages:** `GestureMapView` requests disallow-intercept on touch down, so a drag or pinch on a map never scrolls or stretches the page. The trip map composites over the canvas route and becomes visible only once its style and route are loaded.
- **Controls row:** `SessionControls` measures the labelled row first and drops the Target/Limit/Drive labels when it doesn't fit (narrow screens, large fonts).
